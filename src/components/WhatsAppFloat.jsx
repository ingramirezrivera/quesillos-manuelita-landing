...
export default function WhatsAppFloat() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsOpen(isBottom);
      if (!isBottom) setShowZoneOptions(false);
    };
    ...
    <div className="fixed bottom-6 right-6 z-30 flex flex-col items-end gap-4 pointer-events-none">
      ...
      >
        ...
        <div className="bg-white p-4">
          {!showZoneOptions ? (
            <button
              onClick={() => setShowZoneOptions(true)}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                {whatsappOptions.map((option) => (
                  <a
                    key={option.id}
                    href={`https://wa.me/${option.phone}?text=${encodeURIComponent(
                      option.message
                    )}`}
                    onClick={() =>
                      trackWhatsAppClick({
                    ...
                    className="inline-flex items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-2.5 px-3 rounded-full hover:bg-[#20bd5a] transition-colors w-full shadow-lg shadow-green-500/30 text-sm"
                  >
                    {option.label}
                  </a>
                ))}
